...
const I18N = (() => {
  ...
  const EN = {
    ...
    'r.refreshing': '⟳ Checking…',
    'r.refreshNew': '✓ Loaded latest result — {date}',
    'r.refreshSame': '✓ Already up to date — no newer result yet',
    'r.refreshFail': '⚠ Couldn’t reach the server — check your connection and try again',
    ...
    // library (千字图 / 万字图)
    'l.loading': 'Opening the chart library…',
    'l.none': 'The chart library hasn’t been published yet.',
    'l.error': '⚠ Couldn’t load the chart library — check your connection and try again.',
    'l.locked': '🔒 The chart library is private. Open the unlock link you were sent, or enter the passphrase.',
    'l.bad': 'That passphrase didn’t work — the library may have been re-locked with a new one. Ask for the new link.',
    'l.badLink': '⚠ This unlock link doesn’t work (the passphrase may have changed)',
    'l.passPh': 'Passphrase', 'l.unlock': 'Unlock',
    'l.unlocked': '✓ Chart library unlocked on this device',
    'l.searchPh': 'Search: 蛇, 车, 结婚, snake… or 1234',
    'l.found': '{n} match(es)', 'l.capped': '(showing the first 300)',
    'l.noHits': 'Nothing found for “{q}”.',
    'l.browseH': 'Browse a chart',
    'l.meta': '{n} entries · updated {date}',
    'l.lock': 'Remove from this device',
    'l.lockConfirm': 'Remove the unlocked chart library from this device? You will need the unlock link again.',
    'l.viaLast3': 'last 3 digits ·{d}',
    'l.viaEnding': 'numbers ending in {d}',
    'l.whenWon': 'When did this number win?',
    'l.inCharts': 'In the charts',
    'l.checkLocked': '🔒 Chart meanings are locked — <a href="#" class="goto-library">unlock the library</a>.',
    // watchlist
    ...
    'a.h.vsFair3': 'vs fair 1/1,000',
    'a.bet3top1': 'Ending matches 1st prize', 'a.bet3top3': 'Ending matches any top-3',
    'a.bet3any': 'Ending matches any of 23 prizes',
    'a.oneIn1k': '1 in 1,000', 'a.threeIn1k': '3 in 1,000', 'a.t23of1k': '\u2248 23 in 1,000 (2.3%)',
    'a.check3prize': 'Operators run separate 3D games with their own prize tables \u2014 check the official sites for payouts.',
    'a.ibox3': 'Covering all {n} arrangements of your 3 digits (3D box) multiplies the chance by {n} \u2014 and divides the prize by the same factor. The expected return never changes.',
    'a.reality3': '<strong>Reality check:</strong> these percentages are sampling noise, not an edge. In a fair lottery every 3-digit ending has exactly a 1-in-1,000 chance per prize, every draw, regardless of history.',
    'a.fairNote4': '1.00e-4 (that\u2019s 0.0001 = 1-in-10,000)',
    'a.fairNote3': '1.00e-3 (that\u2019s 0.001 = 1-in-1,000)',
    'pos.axis': 'Position {n}',
    ...
    'a.exBody': '<p>The history contains {total} winning numbers in total, spread across {poss} possible combinations. So if the lottery is fair, an <em>average</em> one should have won about {total} ÷ {poss} ≈ <strong>{n} times</strong> by now. That’s the "expected" figure.</p><div class="ex-example">A ratio like <strong>×1.34</strong> just means "this one has won 34% more often than that average so far" — the same way one friend in a group has usually won more card games than the others. It’s a record of past luck, not a power carried into the next draw.</div><p>And the weekday score table: "P(number | day)" is the model’s guess of the chance on each draw day, built from digit frequencies on that day. Fair value is {fairNote}. The green/grey percentage shows how far the guess sits above or below that fair value — always within noise distance.</p>',
    // weekday
    ...
    'p.sub': 'Two models, both recency-weighted (2-year half-life) and trained on {n} past {day} draws{scope}. Each pick shows the model’s estimated probability against the fair 0.01% baseline; the grey figure is leftover noise, not an edge you can bet on.',
    ...
    'p.chipProb': '{p}% <span class="dim">vs 0.0100%</span>',
    'p.chipNoise': 'noise {sign}{pct}%',
    'p.edgeNote': 'Each pick shows the model’s own probability against the fair 0.01% baseline, and the leftover gap in grey. That gap fails as a betting signal twice over. <strong>First, it is almost certainly not real:</strong> gaps this size fall out of counting a finite history even when nothing is there, the same way 100 coin flips rarely land exactly 50/50 — and the backtest below shows they do not survive out-of-sample. <strong>Second, even if one were completely real, it would still lose money:</strong> the payout table needs roughly +56% before a RM1 bet breaks even, and nothing in this column comes close. There is no value in the grey figures that would make betting profitable.',
    ...
    'p.t.band': 'pure-luck range', 'p.t.bandD': '90% of random runs land in here',
    'p.h.rank': '#', 'p.h.model': 'Model', 'p.h.hits': 'Hits', 'p.h.vsRandom': 'vs random',
    'p.h.sig': 'Verdict',
    'p.sig.inside': 'tied with random', 'p.sig.above': 'beat random', 'p.sig.below': 'worse than random',
    ...
    'p.l.band': 'Pure-luck range',
    'p.verdictTied': 'Top of the leaderboard is <strong>{name}</strong> at ×{ratio} — and that means nothing, because pure random guessing scores anywhere from ×{lo} to ×{hi} over {n} draws. Every model above finished <strong>inside the luck range</strong>, i.e. statistically tied with guessing. Watch what happens when new draws arrive: the ranking reshuffles, and a different model is "best" each time. That reshuffling <em>is</em> the finding — a fair lottery lets models compete and never lets one actually win.',
    'p.verdictBeat': '<strong>{name}</strong> scored ×{ratio}, outside the ×{lo}–×{hi} range that pure guessing produces over {n} draws. Before believing it: with three models tested, one clearing a 90% band now and then is expected — re-run as new draws land and see whether it holds, or falls back into the pack like every previous leader. A single sample outside a band is a hint to keep testing, never a strategy to bet on.',
    'p.rp.calc': 'Checking every draw in the dataset…',
    'p.rp.win': 'Looked back over', 'p.rp.obs': 'Repeats seen', 'p.rp.exp': 'Expected by chance',
    'p.rp.ratio': 'Ratio', 'p.rp.z': 'z (SDs off)',
    'p.rp.lastN': 'previous {n} draws',
    'p.rp.none': '<strong>No cooldown exists.</strong> Every ratio sits at ×1.00 and every z is inside ±2 — meaning numbers that just won come back at exactly the rate blind chance predicts. Avoiding recent winners, chasing "overdue" numbers, and betting on a pattern of any kind all have identical expected value, because there is no memory in the machine to exploit.',
    'p.rp.signal': 'One or more windows is more than 2 SDs from chance. With several windows tested this still happens by luck fairly often, but it is worth re-checking as more draws arrive — a persistent, growing deviation would be the one genuinely interesting result this app could produce.',
    // banner + share
  ...
  const ZH = {
    ...
    'nav.library': '字图',
    'l.sub': '传统字图 — 大伯公千字图、观音千字图、万字图。可搜索字词（蛇、车、snake…）或3–4位号码。',
    'l.note': '字图是马来西亚华人的民俗文化。号码在字图里的意思不会影响开奖 — 每个号码仍然是万分之一。',
    'l.loading': '正在打开字图…',
    'l.none': '字图资料库还没有发布。',
    'l.error': '⚠ 无法载入字图 — 请检查网络后重试。',
    'l.locked': '🔒 字图资料库是私人的。请打开收到的解锁链接，或输入密码。',
    'l.bad': '密码不对 — 资料库可能已换了新密码，请索取新链接。',
    'l.badLink': '⚠ 这个解锁链接无效（密码可能已更换）',
    'l.passPh': '密码', 'l.unlock': '解锁',
    'l.unlocked': '✓ 字图已在本机解锁',
    'l.searchPh': '搜索：蛇、车、结婚… 或 1234',
    'l.found': '找到 {n} 条', 'l.capped': '（只显示前300条）',
    'l.noHits': '找不到“{q}”。',
    'l.browseH': '浏览字图',
    'l.meta': '共 {n} 条 · 更新于 {date}',
    'l.lock': '从本机移除',
    'l.lockConfirm': '要从本机移除已解锁的字图吗？之后需要重新打开解锁链接。',
    'l.viaLast3': '后3位 ·{d}',
    'l.viaEnding': '以 {d} 结尾的号码',
    'l.whenWon': '这个号码几时中过？',
    'l.inCharts': '字图里的意思',
    'l.checkLocked': '🔒 字图已上锁 — <a href="#" class="goto-library">解锁字图</a>。',
    'r.drawDate': '开彩日期', 'r.latest': '最新', 'r.prev': '‹ 上一期', 'r.next': '下一期 ›',
    'r.refresh': '⟳ 检查最新成绩',
    'r.refreshing': '⟳ 检查中…',
    'r.refreshNew': '✓ 已载入最新成绩 — {date}',
    'r.refreshSame': '✓ 已是最新 — 暂无更新的成绩',
    'r.refreshFail': '⚠ 无法连接服务器 — 请检查网络后重试',
    'r.aboutLiveH': '关于“实时”成绩',
    'r.aboutLiveP': '本应用读取 <code>data/draws.json</code>。数据会在每个开彩日（三、六、日晚，另有特别开彩的星期二）之后自动刷新；应用也会每隔几分钟、以及每次重新打开时自动检查新数据。如果成绩仍未出现，点上方的 <strong>⟳ 检查最新成绩</strong> 直接从服务器拉取，绕过设备上的缓存。',
    ...
    'a.h': '号码分析器', 'a.sub': '输入4位号码（或3位数字分析一个3D字尾），放进全部历史数据里检验：实际出现频率、间隔、星期规律和理论概率。',
    'a.btn': '分析',
    'w.chiH': '星期几有影响吗？（χ²检验）',
    ...
    'p.exEdgeSummary': '灰色的“噪音 +20%”是什么意思？',
    'p.exEdgeBody': '<p>0.0100%是公平彩票的基准线：10,000个号码每个都是万分之一。某个选号显示<strong>0.0120%</strong>、<strong>噪音 +20%</strong>，意思是模型的估计值比公平值高出五分之一。</p><div class="ex-example">换成投注语言：模型认为这个号码大约是1/8,300而不是1/10,000。这里标注为<em>噪音</em>而不是<em>优势</em>，是因为历史数字分布永远不会完全平均 — 就像抛100次硬币很少正好50/50。这种不平均是运气，而回测也证实它不会延续到新的开彩上。</p><p>还有真正一锤定音的一点：<strong>就算这+20%完全属实，照样是亏钱的。</strong>RM1的大彩投注平均只拿回约RM0.64，所以大约需要<strong>+56%</strong>才能回本 — 而那一列里的每个数字都远远不到这个门槛。灰色数字不是“小优势”，它根本不是优势。</p>',
    'p.exBtSummary': '回测是怎么做的？“随机选号的期望命中”是哪来的？',
    ...
    'a.h.vsFair3': '对比公平的1/1,000',
    'a.bet3top1': '字尾对中头奖', 'a.bet3top3': '字尾对中任一头三奖',
    'a.bet3any': '字尾对中23个奖项任一',
    'a.oneIn1k': '千分之一', 'a.threeIn1k': '千分之三', 'a.t23of1k': '\u2248 千分之23（2.3%）',
    'a.check3prize': '各公司的3D玩法有各自的奖金表 \u2014 派彩请以官网为准。',
    'a.ibox3': '覆盖这3位数字的全部{n}种排列（3D box），中奖概率乘以{n} \u2014 奖金也除以{n}。期望回报永远不变。',
    'a.reality3': '<strong>现实提醒：</strong>这些百分比是抽样噪音，不是优势。公平彩票中，无论历史如何，每个3位字尾每期每个奖项的概率都正好是千分之一。',
    'a.fairNote4': '1.00e-4（即0.0001 = 万分之一）',
    'a.fairNote3': '1.00e-3（即0.001 = 千分之一）',
    'pos.axis': '第{n}位',
    'ab.dataH': '当前数据集',
    ...
    'a.exBody': '<p>历史数据共包含{total}个中奖号码，分布在{poss}种可能组合上。如果彩票公平，一个<em>平均</em>组合到现在应该中过 {total} ÷ {poss} ≈ <strong>{n}次</strong>。这就是“期望”值。</p><div class="ex-example">像<strong>×1.34</strong>这样的比率只是说“它到目前为止比平均多中了34%” — 就像朋友圈里总有一个人打牌赢得比别人多。它记录的是过去的运气，不是带进下一期的能力。</div><p>至于星期评分表：“P(号码 | 星期)”是模型根据该开彩日的数字频率猜测的当天概率。公平值是{fairNote}。绿色/灰色的百分比显示猜测比公平值高或低多少 — 始终在噪音范围内。</p>',
    ...
    'p.sub': '两个模型都采用近期加权（2年半衰期），基于{n}期过去的{day}开彩训练{scope}。每个选号显示模型估计的概率对比公平基准0.01%；灰色数字是残留的噪音，不是你能拿去投注的优势。',
    ...
    'p.chipProb': '{p}% <span class="dim">对比 0.0100%</span>',
    'p.chipNoise': '噪音 {sign}{pct}%',
    'p.edgeNote': '每个选号显示的是模型自己算出的概率、公平基准 0.01%，以及灰色的差距。这个差距作为投注信号有两重致命问题。<strong>第一，它几乎肯定不是真的：</strong>即使背后什么规律都没有，仅仅统计有限的历史也会产生这种量级的差距，就像抛100次硬币很少正好50/50 — 下面的回测会证明它们在样本外站不住脚。<strong>第二，就算其中某个差距完全属实，照样是亏钱的：</strong>按赔付表计算，RM1的投注大约需要+56%才能回本，而这一列里没有任何数字接近这个门槛。灰色数字里不存在任何能让投注变得有利可图的数值。',
    'p.testing': '回测中…',
    'p.t.replayed': '回放期数', 'p.t.topk': '每期选前{k}个号码，任何奖项都算',
    'p.t.randExp': '随机选号的期望命中', 'p.t.bar': '每个模型都必须跨过的及格线',
    'p.t.band': '纯运气范围', 'p.t.bandD': '90%的随机结果都落在这里',
    'p.h.rank': '#', 'p.h.model': '模型', 'p.h.hits': '命中', 'p.h.vsRandom': '对比随机',
    'p.h.sig': '结论',
    'p.sig.inside': '与随机打平', 'p.sig.above': '赢过随机', 'p.sig.below': '不如随机',
    ...
    'p.l.band': '纯运气范围',
    'p.verdictTied': '排行榜第一是<strong>{name}</strong>，×{ratio} — 但这说明不了任何事，因为在{n}期样本下，纯靠瞎猜的成绩就能落在×{lo}到×{hi}之间。上面每个模型都落在<strong>运气范围之内</strong>，也就是在统计上和瞎猜打平。等新开彩进来再跑一次：排名会重新洗牌，每次“最好”的都是另一个模型。这种洗牌<em>本身</em>就是结论 — 公平彩票让模型互相竞争，但从不让任何一个真正赢。',
    'p.verdictBeat': '<strong>{name}</strong>拿到×{ratio}，超出了{n}期样本下瞎猜会产生的×{lo}–×{hi}范围。但先别急着相信：同时测试三个模型时，偶尔有一个冲出90%区间本来就是意料之中的。等新开彩进来再跑，看它是继续保持，还是像之前每一个“领先者”一样掉回原形。单次超出区间只是继续观察的提示，绝不是可以下注的策略。',
    'p.rp.calc': '正在检查数据集中的每一期…',
    'p.rp.win': '回看范围', 'p.rp.obs': '实际重复次数', 'p.rp.exp': '随机应有次数',
    'p.rp.ratio': '比值', 'p.rp.z': 'z（偏离标准差）',
    'p.rp.lastN': '前{n}期',
    'p.rp.none': '<strong>不存在“冷却期”。</strong>所有比值都停在×1.00，所有z都在±2以内 — 也就是说，刚中过的号码再次开出的频率，和纯随机预测的完全一样。避开近期中奖号、追“很久没开”的号、或者押任何一种规律，期望值都完全相同，因为机器根本没有记忆可供利用。',
    'p.rp.signal': '有一个或多个回看范围偏离随机超过2个标准差。同时测多个范围时，这种情况靠运气也相当常见，但值得随着新开彩继续复查 — 如果偏离持续存在并不断扩大，那将是这个应用唯一可能得出的、真正有意思的结果。',
    'p.rp.h': '中过的号码会避开再次开出吗？',
    'p.rp.sub': '4D最流行的说法是：刚中过的号码已经“用掉了”，短期内不会再来。这个说法是可以检验的，所以这里就用数据集里的每一期来检验它：号码实际重复的频率，对比纯随机应有的频率。如果“冷却期”真的存在，比值会明显低于×1.00。',
    'p.rp.exSummary': '为什么号码中奖后不需要“休息”？',
    'p.rp.exBody': '<p>因为4D是<strong>有放回</strong>抽取的。每一期开完，所有球都放回机器里，所以每一期开始时机器的状态完全相同 — 它并不记得上周开出过什么。</p><div class="ex-example">对比一下真正有效的21点算牌：牌是<em>不放回</em>发出的，所以抽走一张A，确实改变了牌靴里剩下的牌。算牌在4D上失效只有一个结构性原因 — 这里从来没有任何东西被抽走。</div><p>z这一列是实际值与期望值的差距，单位是标准差。粗略地说：|z|小于2是普通噪音，大于3才是真正的异常。如果“冷却期”真的存在，这些基于数万期开彩算出的数字，一定会清楚地显示出来。</p>',
