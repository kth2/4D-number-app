...
const I18N = (() => {
  ...
  const EN = {
    ...
    'c.err34': 'Enter 4 digits (straight check) or 3 digits (3D ending check).',
    'c.sum3': '3D check: winning numbers ending in <strong>{num}{perm}</strong> appeared <strong>{n}</strong> time(s) — {top3} in the top-3 prizes — between {from} and {to}.',
    'c.never3': 'No winning number ending in <strong>{num}</strong>{perm} in the loaded history. With {draws} draws that is unremarkable.',
    'c.h.ending': 'Ending',
    ...
    'a.big': 'Big', 'a.small': 'Small', 'a.days': 'd',
    'pos.axis': 'Position {n}',
    'ab.dataH': 'Current dataset',
    'ab.dataP': '{total} draws · {from} → {to} · source: <strong>{source}</strong>{gen}',
    'ab.dataGen': ' · generated {date}',
    'ab.sampleWarn': '<strong>You are looking at synthetic sample data.</strong> It reproduces the real draw calendar and prize structure so every feature works, but the numbers are simulated. Run the scrapers in <code>scrapers/</code> to replace it with real history.',
  ...
  const ZH = {
    ...
    'wl.h': '我的号码', 'wl.sub': '只保存在本机。可添加4位号码，或只输入3位数字来关注一个3D字尾。显示每项在所选开彩日期的成绩。',
    ...
    'c.h': '这个号码几时中过奖？', 'c.sub': '输入4位数字查正字中奖记录 — 或只输入3位数字，查找所有以这3位结尾的中奖号码（3D玩法规则）。',
    'c.btn': '查询', 'c.perm': '包含所有排列（i-Box / 3D box玩法）',
    ...
    'c.err34': '请输入4位数字（正字查询）或3位数字（3D字尾查询）。',
    'c.sum3': '3D查询：以<strong>{num}{perm}</strong>结尾的中奖号码在 {from} 至 {to} 之间共出现 <strong>{n}</strong> 次，其中 {top3} 次是头三奖。',
    'c.never3': '已载入的历史中没有以<strong>{num}</strong>{perm}结尾的中奖号码。在{draws}期里这很正常。',
    'c.h.ending': '字尾',
    ...
    'a.big': '大万', 'a.small': '小万', 'a.days': '天',
    'pos.axis': '第{n}位',
    'ab.dataH': '当前数据集',
    'ab.dataP': '共{total}期 · {from} → {to} · 来源：<strong>{source}</strong>{gen}',
    'ab.dataGen': ' · 生成于 {date}',
    'ab.sampleWarn': '<strong>你正在查看的是合成示例数据。</strong>它复制了真实的开彩日历和奖项结构，让所有功能都能运行，但号码是模拟的。请运行 <code>scrapers/</code> 里的爬虫来换成真实历史数据。',
